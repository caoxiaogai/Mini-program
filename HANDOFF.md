...
业务页面通过 `services/` 请求 aisales 后端真实接口，由 service 层映射为页面 ViewModel。排行榜后端尚未提供对应接口，页面继续展示既有空状态。
...
- 为列表、详情、统计、用户信息等动态内容准备类型定义。
- 为加载、空数据、错误和正常状态预留可展示的 UI 结构；具体状态以设计稿和用户确认为准。
- 通过 `services/` 统一接入 aisales 后端接口，页面只消费 ViewModel。

### 本期不包含

- 排行榜后端接口（aisales 当前未提供，页面为空态）。
- 真实支付、订单提交等业务闭环。
...
页面不应直接散落 `wx.request` 调用。由 `services/` 统一处理请求、响应转换和错误归一化。
...
- `services/` 暴露页面调用的方法，统一经 `request.ts` 请求 aisales 后端，并映射为页面 ViewModel。
- 页面只消费 service 返回的数据，不直接发起 `wx.request`，也不依赖 mock 文件。
- UI 展示模型与后端响应不一致时，由 service 层完成转换。
- 后端尚未提供的能力（当前仅排行榜）保留 `TODO(API)`，页面展示既有空状态。
...
| 接入后端真实接口 | done | 统一 `services/request.ts` 请求层 + 微信登录；首页/分析/通知/素材全部改为后端数据，视觉验收 mock 已删除；排行榜因后端无接口保留空态占位 |
...
- 页面不直接发起 `wx.request`，业务数据经 `services/` 接入已确认后端接口。
- 尚未提供的后端能力通过 `TODO(API)` 搜索定位（当前仅排行榜）。
...
- 验证：本机 `127.0.0.1:8080` 与 `192.168.31.225:8080` 的 login 均可达（无效 code 时 HTTP 200 + 业务 401）；回归测试 29 项需编译后在开发者工具确认登录不再出现 502。

### 2026-08-25：首页/分析/通知切回真实接口并删除假数据

- 删除 `miniprogram/mocks/` 及 `HOME_DATA_SOURCE` / `ANALYSIS_DATA_SOURCE` / `NOTIFICATION_DATA_SOURCE` 开关；页面继续只消费既有 ViewModel。
- 首页：`/analysis/dashboard` + customer/content/intent 列表组合；通知：`/analysis/intent/list`；分析：dashboard/content/customer/intent 并行。素材发布/列表/详情此前已走真实接口，保持不变。
- 排行榜后端仍无对应接口，`services/ranking.ts` 返回空榜单并保留 `TODO(API)`，页面展示既有空状态。
- 验证：完整测试 `node --disable-warning=MODULE_TYPELESS_PACKAGE_JSON --test tests/home-page.test.mjs` 29 tests passed；`git diff --check` 通过。未改动 WXML / Less / JSON。开发环境后端 `http://192.168.31.225:8080/api` 可达（`GET /analysis/dashboard` HTTP 200）。当前环境未安装项目级 `tsc`，未执行独立 TypeScript 编译。
