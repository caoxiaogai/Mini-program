...
- 通知和分析的固定顶部 Header 使用白色实色端与白色透明端。素材页和首页内嵌素材 Tab 已删除彩色 `publish-header-gradient.png` 图层，改为 136px 高的白色渐变：顶部 `rgba(255, 255, 255, 0.2)`、底部 `#FFFFFF`，使白色渐变在素材滚动到顶部时保持可见。
