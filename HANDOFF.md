...
### 2026-08-25：「今日最多」有数据时跳转作品分析

- 有数据时，首页「今日最多」内容卡整体点击切换到分析页「作品分析」Tab；移除该区域原先进入内容详情的子项点击路径。
- 空状态仍保留「立即发布」按钮，点击不会进入分析页。
- 验证：定向回归通过。
...
- 验证：总数据两项回归通过，JSON 解析与 `git diff --check` 通过；全量回归当前为 `50 passed / 1 failed`，失败项是并行新增的个人页测试引用尚不存在的 `miniprogram/types/profile.ts`，与本次改动无关。本工作区未安装 TypeScript CLI，需在微信开发者工具中完成最终编译与视觉复核。
