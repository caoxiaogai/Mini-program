...
- 发布接口仍通过现有连续 `media` ViewModel 提交；后端当前没有槽位字段。
- 验证：`node --experimental-strip-types --test tests/publish-material-entry.test.mjs` 13 项通过；当前工作区未安装 `tsc`，无法执行独立 TypeScript 编译。
