...
### 2026-09-09：存草稿 / 发表时显示「文件上传中」

- 发布页、笔记页点「存草稿」或「发表」后，只显示页面上的「文件上传中」，不再叠一层系统「加载中」。
- 动画一直保持到文件上传、落库和分享卡都走完，成功则直接跳转，失败才关掉动画。

### 2026-09-08：朋友圈单页模式点按钮提示前往小程序

- 单页模式不能跳转或登录。点任何按钮都 `toast`「请前往小程序」，引导使用微信底部的「前往小程序」。
- 首次用户仍先看到前置页；已登录用户仍可滑动查看作品，只是按钮被拦住。

### 2026-09-08：朋友圈单页模式首次仍显示前置页

- 点朋友圈卡片会先进入微信单页模式，不能跳转或登录。
- 首次用户在单页模式直接画出前置页，不再加载作品或首页骨架。
- 已登录用户在单页模式仍直接看作品。
- 点「前往小程序」进入完整小程序后，首次用户走前置页 → 头像昵称授权 → 作品详情。

### 2026-09-08：首次进入仍先前置页再授权

- 未完成头像昵称的首次用户，无论从普通入口、好友分享还是朋友圈进来，都先到前置页，再进授权头像昵称页。
- 授权完成后，作品分享进入对应详情，普通进入进入首页。
- 已登录用户跳过这两页，朋友圈和分享直接打开作品详情。
- 前置页和授权页不再等登录接口才渲染，避免一直停在空白加载。

### 2026-09-08：分享卡按素材列表的 aspectFill 裁切

- 列表缩略图仍显示原封面，由卡片框 `aspectFill` 裁切，不预先改图。
- 发给好友和朋友圈的 `imageUrl` 先按列表预览框居中铺满裁切，再放进微信要求的 5:4 卡片，避免平台按另一套比例再裁。
- 画布生成失败时回退原封面。

### 2026-09-08：作品详情底栏更多改为文字并收紧间距

- 「更多」不再用三点图标，直接显示「更多」两字，仍靠右。
- 点赞、转发、评论不再均分整行，图标间距收紧。

### 2026-09-08：微信输入法不再反复弹收键盘

- 候选栏会把键盘高度打成 0，并误发 blur；输入过程中不再因此收起或重绑 `focus`。
- 键盘升起后忽略高度抖动，输入时不 `setData` 文案和滚动，避免受控 `textarea` 把输入法冲掉。
- 只有点「完成」、点「+」，或键盘高度持续为 0 后，才真正收起。

### 2026-09-08：笔记键盘工具栏左侧增加完成

- 「+」那一栏最左侧增加「完成」；点击后收起键盘，回到存草稿 / 发表。

### 2026-09-08：笔记页进入即展开加号面板

- 打开笔记编辑页时，底部直接显示照片、拍摄、位置、文件，不必先点「+」。
- 点输入区唤起键盘后仍收起该面板，键盘上只保留「+」。

### 2026-09-08：作品详情底栏改为点赞、转发、评论

- 去掉底栏「转发给好友 / 分享到朋友圈」，以及作者视角的删除、二次编辑并排按钮。
- 底栏改为点赞、转发、评论三个线框图标，图标下显示数量；不足万直接数字，满万用「万」，满 10 万用「万+」。
- 转发仍走原生 `button open-type="share"`，并继续上报转发埋点。
- 作者视角在最右侧显示「更多」，菜单里是二次编辑和删除；访客不显示更多。
- 数量接口：`GET /material/{id}/engagement`、`POST /material/{id}/like`、`GET|POST /material/{id}/comments`。已有库需执行 `sql/material_engagement.sql`。

### 2026-09-08：分享前置页普通进入与作品分享共用底部位置

- 「点击查看更多」「好友赞了该内容」「查看更多」和底部协议文案固定在安全区之上，不再被插画区顶出屏幕。
- 普通进入时气泡插画改在剩余上半区缩放；作品分享时预览图铺满同一块上半区。

### 2026-09-08：分享前置页插画在作品分享时换成列表预览图

- 普通首次进入仍显示默认插画和「好产品 / 值得多了解一点」。
- 从别人分享的作品进入时，整块插画区（插画 + 标题文案）都换成该作品在素材列表上的预览图；拉不到预览则保持默认插画区。

### 2026-09-08：已登录用户不再进入分享前置页和授权页

- 分享前置页、头像昵称授权页只给首次未登录用户。
- 本地已完成登录（已授权且头像昵称有效）时，这两个页面不渲染，直接进入首页或作品详情。
- 朋友圈 `entry=share-gate` 也只在未登录时转到前置页；已登录用户留在作品详情。
- 静默登录若只返回占位头像昵称，不再覆盖本地已完善的资料。

### 2026-09-08：笔记输入时光标始终保持在键盘上方可见

- 键盘和加号栏叠在滚动区底部，长文输入时 `scroll-into-view` 只会滚到段落顶部，光标会被挡住。
- 输入、换行和键盘升起后按光标位置计算 `scroll-top`，把当前行滚到键盘与工具栏上方。
- 只改滚动位置，不回写 textarea 的 `value`/`focus`，避免再触发输入法反复弹收。

### 2026-09-08：笔记仅内容区及下三行可唤起键盘

- 不再点整页空白唤起输入法。
- 无内容时，只有正文前三行（`144rpx`）可唤起。
- 有内容时，可点已有内容，以及最后一行下方三行。

### 2026-09-08：笔记仅内容区和下三行可唤起键盘

- 不再点击整页唤起输入法。
- 无内容时，只有正文前三行（`144rpx`）可点。
- 有内容时，点击已有内容，或最后一行下方三行热区，才会唤起输入法。

### 2026-09-08：笔记点空白唤起键盘不再反复弹收

- 部分机型点空白后，键盘高度会闪一次 0，布局变化又把同一次点击当成再次点空白，造成键盘反复唤起/收起。
- 已聚焦或键盘已升起时不再重复走失焦再聚焦；用户收起键盘时立刻放开 `focus`，并忽略收起引起的误点，避免马上又弹出来。
- 微信输入法会在候选栏变化时把高度打成 0 并在 `setData` 后重新拉起键盘：高度 0 先延迟确认再收起，聚焦后不再重复 setData，输入中不回写受控 `value`，textarea 使用 `hold-keyboard`。

### 2026-09-08：笔记空白态增加输入提示

- 未输入文字、也未添加图片/视频/位置/文件时，输入区显示「记录文字、图片、视频等」。
- 一旦有内容，占位提示消失；清空后重新出现。

### 2026-09-08：笔记选位置失败时回退打开地图

- `wx.chooseLocation` 在拿不到当前定位时会直接失败（开发者工具未模拟位置、系统未授权，或内部依赖未声明的精确定位）。
- 拒绝授权时引导去设置；其他失败会用默认中心点再打开一次地图选点，避免一进来就提示「无法获取位置」。
- 公众平台仍需开通 `chooseLocation`，并在隐私指引中声明「收集你选择的位置信息」。

### 2026-09-07：分享前置页社交证明文案保持单行

- 「你有 6 个好友赞了该内容」不再受 180px 容器宽度限制，使用整行可用宽度并 `white-space: nowrap`，各机型均单行显示。

### 2026-09-07：取消授权页查看协议，首次进入走分享前置页

- 去掉授权页「查看协议 / 同意并继续」自定义页面，进入后直接是头像昵称填写。
- 未登录首次进入业务页仍先到 `pages/share-gate/index`；点「查看更多」进入微信授权头像昵称页。
- 保留 `app.json` 的 `__usePrivacyCheck__`，需要时由微信官方弹窗处理隐私同意。

### 2026-09-07：授权页一键登录先选头像昵称，再变成授权查看

- 「使用微信一键登录」先走原生 `chooseAvatar`，选完头像后自动聚焦昵称框，便于键盘上方出现「微信昵称」。
- 头像和昵称都填写完成后，底部按钮改为「授权查看」，作为真正的登录提交。
- 头像区域仍可单独更换头像。微信不允许在一个非原生按钮上同时弹出头像选择和昵称条。

### 2026-09-07：适配微信隐私保护指引后再使用头像昵称

- 已取消授权页自定义「查看协议」流程；需要时由微信官方弹窗处理隐私同意。
- `app.json` 保留 `__usePrivacyCheck__`。
- 公众平台除「收集你的昵称、头像」外，头像选择器还会走相册，需同时声明「收集你选中的照片或视频信息」。声明审核通过后重新编译预览，并删除小程序后再打开。

### 2026-09-07：首次进入先到分享前置页再授权

- 未登录用户进入业务页时先到 `pages/share-gate/index`，不再直接跳授权页。
- 前置页点「查看更多」再进入微信授权页；有作品 id 时授权后回作品详情，没有则回首页。
- 已登录用户不受影响，退出登录后再次进入也会先看到前置页。

### 2026-09-07 分享入口第二步微信授权页

- 按 Figma `1167:395` 更新现有 `pages/auth/index`：微信授权标题、副文案、左对齐 80px 方形头像、昵称输入框、底部橙色“使用微信一键登录”按钮；尺寸按 393px 设计宽度转换为 rpx。
- “查看更多”统一进入授权页，携带原作品 id / trackingId 对应的 return 路径，继续复用现有登录、头像上传、资料保存和成功返回服务。
- 头像占位使用 Figma 原始导出资源 `assets/auth/avatar-placeholder.svg`；只用于展示，不作为已选择头像提交。头像使用原生 chooseAvatar，昵称使用原生 nickname 输入框。
- 授权页设计变量集中在 app.less；复用导航组件与安全区处理，修正微信默认 button 居中和默认宽度影响。
- 验证：新增查看更多跳转行为测试及 auth-login 测试共 2 项通过。扩展分享/授权相关测试 13 项通过，1 项既有测试仍要求已移除的 step=profile 参数而失败（utils/auth.ts 未修改）。
- 微信开发者工具已加载新页面，实际点击查看更多成功进入授权页，并在 iPhone 15 Pro Max 模拟器目视检查头像左对齐、昵称字段、底部按钮和安全区，调试器显示运行时 Errors: 0。未执行真实头像授权、资料上传与登录提交；登录后回跳的端到端真机验证待完成。编辑器仍显示首页原有 3 项问题，未进行全仓 TypeScript 类型通过声明。


### 2026-09-07 分享授权前置页

- 新增分享授权前置页 `pages/share-gate/index`，参考本地 Figma `1157:11028`。
- 页面使用固定社交证明文案“你有 6 个好友赞了该内容”、六个头像和用户提供的 `Group 98.svg`；“查看更多”按作品 `id` / `trackingId` 进入授权流程。
- 社交证明文案使用 16px；“查看更多”按钮铺满内容区，左右各保留 20px 内边距。
- 按钮文字使用 56px 行高并清除上下内边距，确保垂直居中。
- 顶部插画区改为内容流自适应高度，插画容器同时按设备宽度与视口高度约束并保持等比显示；顶部区与内容区之间合并为单一 32px 间距，按钮与授权说明保持固定 30px 间距，移除原先 550px 固定高度。
- 作品分享卡路径先进入授权前置页；朋友圈分享通过 `entry=share-gate` 标记由作品详情页转入前置页，授权完成后回到对应作品详情。
- 验证：分享授权路由、作品详情分享断言通过；`app.json` / 新页面 JSON 解析和 `git diff --check` 通过。全量页面清单测试仍受仓库既有 `pages/materials/note/index` 清单差异影响，未修改该无关项；本地未安装 TypeScript 编译器。

### 2026-09-07 榜单视觉调整

- 按本地 Figma `1150:10879` 为榜单面板添加 `1px #808080` 描边、`32rpx` 圆角。
- 滚动导航背景由蓝灰色改为纯白，保留原透明度渐变。
- 榜单头部使用用户提供的奖杯 SVG，保存为 `assets/ranking/ranking-trophy.svg`；首页现有奖杯不变。
- 尚未进行微信开发者工具真机视觉验收。
- 验证：榜单背景及共享筛选控件两项定向测试通过；`git diff --check` 与 SVG 原文件一致性检查通过。榜单 Mock 排序测试因缺少 `miniprogram/mocks/ranking.ts` 无法运行，未改动该数据测试。

### 2026-09-07 个人作品详情底部操作固定锚点

- 个人视角的作品详情底栏不再使用 flex 横向分布，改为以 iPhone 16 设计宽度 393px 为基准的四个固定锚点：删除、编辑、好友、朋友圈；避免微信原生 `button` 的默认外边距参与排布后将编辑和分享按钮撑散。
- 操作栏高度包含底部安全区，四个 48px 控件保持在栏顶下方 17px。好友与朋友圈原生按钮同时锁定为 `191rpx` 的最小、目标和最大宽度，并裁切溢出内容，避免微信按内容最小宽度将右侧按钮撑出屏幕。客户访问视角仍使用原有两按钮分享栏，未改动。
- 客户访问视角的双按钮栏补回 `#F0F0F0` 顶部分隔线；两个按钮采用可收缩的等分 flex 基线并裁切溢出，同时显式隐藏原生 `::before/::after`，防止半圆残影和两按钮无间距。
- 验证：新增锚点回归检查，先在旧布局下失败、改动后通过；`git diff --check` 通过。已在开发者工具重新编译并实际打开个人作品详情，运行时识别到四个操作控件；桌面截图权限不可用，仍需在模拟器画面中确认像素级视觉。
...
| 已发布作品二次编辑 | done | 作者在素材详情「更多」里二次编辑，预填媒体和文案进入发布页，发表为新作品 |
| 作品详情点赞转发评论 | done | 底栏改为点赞/转发/评论及数量；作者「更多」含二次编辑和删除，访客不显示更多 |
...
### 2026-09-07：会员上限提示卡只显示真实被挡访客

- 首页互动消息和通知页的「又有 N 个用户查看了你作品」不再用 Figma 5 人占位。
- 通知列表接口会先滤掉被挡访客，人数和头像改从 `GET /membership/me` 的 `hiddenVisitorCount` / `hiddenVisitors` 读取。
- 有被挡访客才出卡；没有则不显示。需重启 `aisales_dev`。

### 2026-09-07：隐藏访客提醒文字调整

- 首页和通知页复用的隐藏访客提醒中，「又有 5 个用户查看了你作品」字号调整为 `14px`，访客数字统一使用主题橙 `#FF8901`。
