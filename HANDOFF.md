...
### 2026-09-25：笔记意向规则与 PDF 相同

- 笔记未看完为低意向，看完为中意向。打开 2 次且至少看完 1 次、看完后又向上翻，或者转发，为高意向。
- 设置页「意向判断标准」增加笔记，文案与 PDF 相同。未看完的笔记通知状态为「未浏览完文件」。

### 2026-09-25：访客离开小程序后再发通知

- 小程序通知列表和服务号推送不再在浏览过程中发出。访客还在小程序里时先记下浏览；离开后再出现在通知里，并按当时的意向发服务号。
- 小程序在前台每 2 秒发一次心跳。心跳停止约 4 秒后视为已离开，因为切到后台时离开请求经常发不出去。离开后马上又打开，这次通知会取消。
- 已有库要先执行 `aisales_dev/sql/2026-09-25-notify-after-leave.sql`，再重新部署 `aisales_dev`。历史通知保持可见。

### 2026-09-25：看完后再回看记为高意向

- 多图看完所有图片后，又滑回前面的图片，记为高意向。PDF 看完后向上翻回之前的内容，同样记为高意向。视频播放进度超过 80% 后又回退，记为高意向。
- 原有规则保留：转发，以及打开 2 次且至少 1 次看完（视频为至少 1 次达到 80%），仍是高意向。
- 回看由小程序上报 `seek`。开发后端 `aisales_dev` 用它计算意向。需要重新部署后，分析页和推送才会按新规则生效。

### 2026-09-25：访客人数上限改由后端配置

- 非会员、普通会员的人数上限写在开发后端 `business.membership.visitor-limit-none` 和 `visitor-limit-regular`，默认仍是 8 和 80。Pro 不限制。
- `GET /membership/me` 返回当前用户的 `visitorLimit`，以及开通页用的 `visitorLimitRegular`。小程序不再写死这两个数。

### 2026-09-24：访客上限按周期重置

- 非会员、普通会员的独立访客名额不再按账号累计。每个重置周期按该周期内的首次浏览时间重新取前 N 人；超出的人在当期看不到记录，下个周期的新访客重新计入，直到再次达到上限。
- 默认每个自然月 1 日 0 点重置。开发后端 `business.membership.visitor-quota-reset` 可改成 `1m`、`30s`、`1h`、`1d` 方便测试，改完需要重新部署 `aisales_dev`。
- 名单由后端按周期过滤。小程序不再对通知和用户分析列表做第二次截断，否则跨周期时会把已放行的访客裁掉。

### 2026-09-24：用户分析列表展示访客上限卡

- 分析页和首页分析 Tab 的意向客户列表顶部，在有未展示访客时显示与首页相同的访客上限卡。
- 「意向客户」括号内改为当前日/周/月筛选下的全部意向客户数，包含被上限挡住、列表里未展示的人。

### 2026-09-24：非会员访客上限提示写明免费额度

- 首页和通知里的访客上限卡，非会员文案为「8个免费额度已用完，升级会员，查看详情」。
- 普通会员仍为「升级会员，查看详情」。
...
- 现在在点击当下调用 `wx.requirePrivacyAuthorize`，同意后再打开相册或文件选择。
- 选图或选文件失败时弹出微信返回的原始 `errMsg`，不再使用「选择失败，请稍后重试」。
- 公众平台「用户隐私保护指引」须已声明「收集你选中的照片或视频信息」和「访问你的摄像头」，否则协议弹窗仍会失败。改完后需重新提审。

### 2026-09-22：非分享进入只在发布和会员时登录

- 不是从别人分享作品进入时，不再进前置页拦登录，直接进入首页游客态，可切换 Tab、看空状态和排行榜。
- 只有点「发布素材 / 发布作品」、开通或升级会员、进入设置，以及未登录时点「我的」里的默认头像或昵称，才打开授权登录。头像和昵称登录后回到「我的」。
- 从别人分享的作品进入仍走前置页 → 授权 → 作品详情。
- 退出登录后回到首页游客态，不再回到前置页。

### 2026-09-13：授权登录页左上角增加返回

- 微信授权登录页左上角使用与素材详情页相同的 `navigation-bar` 返回按钮。
- 点返回回到上一页（素材列表或分享前置页）。

### 2026-09-13：普通进入前置页「查看更多」先到素材列表

- 不是从别人分享的作品进入时，前置页点「查看更多」进入首页「发布」素材列表（空状态），不先进授权登录。
- 浏览筛选和底部 Tab 不要求登录；只有点「发布素材 / 发布作品」、开通或升级会员、进入设置时才打开授权登录（见 2026-09-22）。
- 从别人分享的作品进入（前置页带作品 id）仍保持原逻辑：点「查看更多」进授权，完成后回作品详情。

### 2026-09-11：会员套餐改回正式价

- 后端 `MembershipPlan`（仅 `aisales_dev`）正式标价：标准档 1 个月 ¥38.88、3 个月 ¥88.88、半年 ¥168.88；尊享档 1 个月 ¥80.88、3 个月 ¥208.88、半年 ¥388.88（分：3888 / 8888 / 16888 / 8088 / 20888 / 38888）。
- 页面金额仍来自 `GET /membership/me`。标准档三个月/半年优惠文案按月价折算为 24% / 28%；尊享档为 14% / 20%。
- 微信后台虚拟支付道具标价必须改成一致并重新发布到现网。重启体验后端后会员页显示新价格。已支付订单金额不变。

### 2026-09-11：会员开通成功点「知道了」回到「我的」并刷新

- 开通成功弹窗点「知道了」后回到首页「我的」Tab，并重新拉取个人中心会员卡。
- 返回前直接调用首页实例的 `showProfileTab()`，等切到「我的」后再拉 `GET /membership/me`；不依赖底层页 onShow（Skyline 下可能不触发）。失败、取消、待确认弹窗仍留在会员页。

### 2026-09-10：选图进入创建页不再带上首页加载中

- 从相册选完图后，首页/素材列表的 `onShow` 不再立刻重拉仪表盘和列表。
- 首页和素材列表的后台请求改为静默，避免全局「加载中」跟到创建页。选图本身不处理文件，和图片大小无关。

### 2026-09-10：创建成功弹窗等详情加载后再出现

- 创建成功进入详情后，不再立刻弹出分享窗。
- 详情请求成功并写入页面数据后，再弹出「创建成功」，避免此时点「分享给好友」没有标题、卡片变成当前页截图。
- 详情加载失败时不弹窗。

### 2026-09-10：数据总览内层统计卡不再超出外框

- 总发布等 2×3 统计卡改为三列等分，宽度跟随外框，不再使用固定 98px。

### 2026-09-10：相册选择显示「原图」，由用户决定是否原文件上传

- 选择图片、视频时打开微信相册的「原图」选项，不再强制压缩。
- 勾选「原图」则上传原始文件；不勾选则按压缩文件上传。

### 2026-09-10：标准会员追踪人数改回 80

- 标准档独立访客上限由联调值 10 改回正式值 80，小程序 `MEMBERSHIP_VISITOR_LIMIT_REGULAR` 与开发后端 `MembershipVisitorLimits.REGULAR` 一并更新。
- 未开通仍为 8 人，尊享仍为不限制。

### 2026-09-10：笔记页后退前进按每一步撤销重做

- 编辑文字时立刻记入历史，后退/前进按钮不再一直灰色。
- 每输入、删除、插入图片/视频/位置/文件都是独立的一步；后退和前进按单步还原，不再等收起键盘后整段一起撤销。

### 2026-09-10：笔记页创建按钮改为主题橙

- 笔记编辑页底部「创建 / 修改」按钮背景与图文、视频、PDF 发布页一致，使用主题橙 `#FF8901`，不再使用笔记页原来的青色。

### 2026-09-10：已删除作品在前置页和单页显示默认气泡图

- 进入分享前置页或朋友圈单页时，即使链接带了封面参数，也会请求确认作品是否还在。
- 作品已删除（`1008` / `404` / 「发布者已删除作品」等）时改回本地默认气泡图，不再继续展示卡片封面。
- 网络失败或没有封面时保持当前画面，不把普通错误当成已删除。

### 2026-09-10：朋友圈单页直接显示作品图

- 分享到朋友圈时把远程封面写进页面参数；单页第一帧用这张图铺开，并尽量走微信已缓存的卡片图。
- 旧卡片没有封面参数时，仍先留空再请求封面，不再闪默认气泡图。

### 2026-09-11 通知、发布与分析顶部视觉更新

- 通知和分析的固定顶部 Header 使用白色实色端与白色透明端。素材页和首页内嵌素材 Tab 已删除彩色 `publish-header-gradient.png` 图层，使用 136px 高的白色渐变：顶部至 80px 为 `#FFFFFF`（100%），再向底部的 `rgba(255, 255, 255, 0)`（0%）过渡，使素材列表从顶部白色遮罩下逐渐露出。
- 首页「发布作品 / 待跟进」首屏移除原有顶部灰色和彩色渐变层，使用用户提供的本地主题图 `assets/home-new/home-theme-sunset.jpg`（786×746，128KB）作为最底层；其上覆盖 380px 高的白色渐变，顶部透明度 20%、底部透明度 100%，导航栏、标题与内容保持在主题层上方。主题图只有一个资源入口，后续可直接替换；为满足真机上传 2MB 包体限制，已将原 1.86MB PNG 下采样压缩为 JPEG。
...
- 修改保存成功后立刻刷新栈里的首页素材 Tab 和素材列表；返回列表或点到底部素材 Tab 时也会再拉一次，封面和文案与详情一致。

### 2026-09-10：去掉草稿，修改直接改原作品

- 发布页和笔记页去掉「存草稿」，只保留「创建 / 修改」。
- 列表不再显示草稿标记，点卡片一律进详情。
- 详情更多菜单「二次编辑」改为「修改」，打开原作品编辑页并回写同一条素材，不再新建。
- 修改成功后回到原详情并刷新；新创建仍进入详情并弹出分享窗。
- 后端 `PUT /material/{id}` 现可更新标题、文案和文件；已发布作品再次生成分享链接时不再更换追踪码。

### 2026-09-10：创建后从详情返回列表会刷新

- 创建成功进入详情后再返回，素材列表页和首页素材 Tab 会重新拉取列表，新作品会出现在列表里。

### 2026-09-10：发布页创建成功进入作品详情

- 发布页和笔记页主按钮由「发表」改为「创建」，上传遮罩由「文件上传中」改为「创建中」。
- 创建成功后 `redirectTo` 打开该作品详情，不再回到列表。
- 详情页弹出发布成功窗，恢复「分享到朋友圈」；点击后引导使用右上角「···」里的朋友圈分享。详情页本身仍是唯一开启朋友圈菜单的页面。

### 2026-09-10：分享开屏页底部区域按 Figma 1277:6628 更新

- 提示文案改为 `#666666` 14px；保留项目内六个本地头像，点赞文案统一为「以上好友赞了该内容」。
- 好友头像增加 `blur(3px)`，仅隐藏头像细节，不改变头像尺寸、重叠关系或白色分隔边框。
- 提示、好友区和行动按钮的间距统一为 20px，头像与文案间距为 10px；好友区采用 Figma 的 180px 居中宽度。
- 「查看更多」改为 48px 高、`#F0F0F0` 底色、`#E0E0E0` 描边和主题橙 `#FF8901` 文案的圆角按钮；左右 20px 保持不变，底部改为固定距离屏幕 32px，不再叠加 iOS 安全区。
- 底部内容背景固定为 400px 高：由底部起 180px 保持 `#FFFFFF` 不透明，再向上渐变到透明，从而自然衔接上方浅灰插画区域。
- 默认插画和分享作品封面均作为整屏背景层，容器固定覆盖 `100vw × 100vh`，使用 `aspectFill` 保持比例铺满屏幕；底部渐变内容层覆盖在背景之上。
- 移除默认插画背景中的「好产品 / 值得多了解一点」标题文案及其无用容器和样式。
- 默认背景替换为用户提供的 `default-background.png`（1179×2556），其中包含完整插画与标题；所有前置页入口统一使用该本地资源。
- 前置页导航栏背景统一透明，消除默认 `#F8F8F8` 顶部色块，使背景图连续铺满状态栏以下区域。
- 验证：`node --experimental-strip-types --test --test-name-pattern="share gate" tests/home-page.test.mjs` 2 项通过，`git diff --check` 通过；尚未在微信开发者工具进行本次视觉验收。

### 2026-09-10：分享开屏页移除底部协议文案

- 移除「微信安全协议保护，内容仅对授权用户可见」文案及其样式。
- 共享开屏组件删除文案后，「查看更多」按钮成为内容末项；其后续底部位置以最新 Figma 1277:6628 的固定 32px 规则为准。
- 验证：`node --experimental-strip-types --test --test-name-pattern="share gate" tests/home-page.test.mjs` 2 项通过，`git diff --check` 通过。
- 有作品时封面仍走 `getMaterialListPreview`；无作品或接口无图时才用本地默认底图，不把远程装饰头像当成真实点赞数据。


### 2026-09-09：通知页一键已读点击生效

- 完整「一键已读」按钮上的叉和文字原先走首页紧凑交互的 `catchtap`，点下去不会标记已读。
- 通知页和首页通知 Tab 的完整按钮，点文字或叉都会执行一键已读；首页标题旁的紧凑叉仍是先展开再点文字。

### 2026-09-09：好友分享卡图片铺满图片区

- 转发给好友、分享到朋友圈的卡片图按微信 5:4 居中铺满裁切，不再按列表竖图比例放入卡片，避免两侧留白。
- 列表缩略图仍用原封面 + `aspectFill`，不受影响。

### 2026-09-09：发布页图片支持九宫格长按拖拽

- 仅多张图片支持最多 9 个槽位：页面只渲染已有图片和一个「+」，例如 6 张图时不渲染第 7–9 个空槽。长按任意图片进入拖动状态，只能移动到已有图片并直接互换；隐藏空槽和「+」均不可作为拖拽目标。
- 长按后以悬浮预览图跟随手指移动：原格半透明保留，目标格只高亮；松手时才交换并以 160ms 动画落入目标格，取消手势则回到原格。视频与 PDF 仍维持原有单文件行为，新增、删除、上传、存草稿与发表逻辑不变。
- 发布编辑页的图片缩略图支持轻点预览：按下时短暂缩小并降低透明度，随后调用微信原生全屏图片预览；长按拖拽后的点击会被忽略，避免误打开预览。
- 发布编辑页底部「发表」按钮背景统一为主题橙 `#FF8901`；尺寸、圆角、白色文字与现有按压反馈不变。
- 草稿加载、上传、存草稿、发表和分享卡仍走本地已接的 `getMaterialDraft`、`uploadMaterialFiles`、`saveMaterialDraft`、`publishMaterial`、`getMaterialShareCard`；拖拽只改本地槽位顺序，提交仍是连续 `media`，没有假数据。


### 2026-09-09：评论页点空白收起键盘

- 作品详情评论弹层唤起键盘后，点标题、列表空白或评论内容会收起键盘，评论层保持打开。
- 「发送」和输入框本身不收键盘；点遮罩或「关闭」仍关掉评论层。

### 2026-09-09：标准会员卡片按 Figma 1256:5977 更新

- 标准会员状态保留 160px（`320rpx`）卡片高度、40rpx 内容边距和动态追踪进度逻辑；底图替换为本地 Figma 导出的暖色渐变卡片。
- 「标准会员」使用 `#D76E36 → #AA5018` 渐变字；到期时间、升级提示、追踪文字、已使用进度和箭头统一为 `#8B4A26`，未使用进度为白色。
- 卡片种类、到期时间和剩余追踪人数继续来自 `GET /membership/me`；开发者工具不再用假数据覆盖真实档位。

### 2026-09-09：尊享会员卡片按 Figma 1257:6281 更新

- 尊享会员状态保留 160px（`320rpx`）卡片高度、40rpx 内容边距、到期信息和续费入口；底图替换为本地 Figma 导出的暖金渐变资源。
- 「尊享会员」使用 `#F89432 → #C6561F → #FF9B0A` 渐变字；到期时间、尊享说明为 `#9E6C56`；续费按钮使用 `#F1AD30 → #FEEFD5` 渐变、`#DAB26F` 描边和 `#481D0D` 文案。

### 2026-09-09：个人中心「尽情期待」按钮主题色更新

- 保持尺寸、圆角和白色文字不变，背景由青色改为主题橙 `#FF8901`。

### 2026-09-09：未开通会员卡片按 Figma 1256:5031 更新

- 未开通会员状态改为紧凑的 68px（`136rpx`）暖色渐变卡片；头像与卡片间距调整为 20px（`40rpx`）。
- 卡片背景、橙色描边、右侧圆环、皇冠与箭头均使用本地 Figma 导出资源；皇冠使用原始 PNG，避免小程序无法渲染 SVG 内嵌位图而显示黑块。标题、行动文案使用 `#8B4A26`，副标题使用 `#B77A32`。
- 删除旧版倾斜外层、遮罩、圆环资源及逐字渐变副标题逻辑；会员点击入口、标准会员与尊享会员卡片保持原行为。

### 2026-09-09：个人页功能列表图标改为主题色

- 「钱包中心、我的收藏、第三项」功能列表图标继续使用现有本地 SVG 与尺寸，描边色统一由 `#00B7E6` 更新为主题橙 `#FF8901`。


### 2026-09-09：仅作品详情页可分享到朋友圈

- 右上角「分享到朋友圈」只在作品详情页开启。
- 首页、素材列表关闭朋友圈分享；发布成功弹窗只保留分享给好友。

### 2026-09-09：PDF 详情预览底部增加点击查看

- 按 Figma `1230:3417`，PDF 作品详情预览图底部增加白色圆角「点击查看」和灰色提示「这是PDF文件，需要点击查看详情」。
- 点预览图或按钮仍进入文档阅读页；表格素材不显示这组 PDF 文案。

### 2026-09-09：发布页加号锁定进入时的素材类型

- 从首页或素材页选「图片 / 视频 / PDF」进入编辑页后，点「+」只继续添加同一类型。
- 把已有素材全部删掉后再点「+」，也不再弹出类型选择，仍按进入时的类型添加。
...
### 2026-09-09 同步 developer-v2

- 快进至 c06c834，恢复本地开发接口配置及用户轨迹标签修改。远程底栏已替换为点赞/转发/评论，客户视角去线通过新底栏 visitor 样式保留，作者边线不变。

### 2026-09-09 作者作品详情底栏按 Figma 1236:3521 更新

- 仅作者视角：左侧改为「更多操作 / 编辑和删除作品」入口，继续复用原有二次编辑和删除菜单；右侧改为转发、点赞、留言三项，图标使用用户提供的 SVG，尺寸为 24px。
- 作者底栏底部间距固定为 24px（不叠加 iOS 安全区）；转发文字显式使用常规字重，与点赞和留言一致。
- 作者与客户右侧的转发、点赞、留言统一按各自互动数量切换：数量为 0 显示动作名，数量大于 0 显示对应数量；互动文字共享黑色、22rpx 字号、常规字重（400）与 28rpx 行高。
- 点赞后的图标统一为 `#FC1616` 实心爱心，替换此前的大拇指资源；状态切换时执行一次 `0.72 → 1.24 → 1` 的 260ms 回弹动画。
- 客户视角底栏按 Figma 1250:4518 更新：左侧为「说点什么」留言入口，点击复用现有留言抽屉；右侧按转发、喜欢、留言顺序展示，三项互动逻辑与作者一致。入口使用 Figma 导出的本地编辑图标，底栏使用 `#F4F5F5` 顶部分隔线，底部间距固定为 24px（不叠加安全区）。作者与客户底栏共用固定 10px 顶部间距，互动项不再叠加额外上下内边距。
- 验证：作者与客户底栏定向回归测试通过；已在 iPhone 12/13 模拟器重新编译「客户视角详情」，确认显示「说点什么」与转发、点赞、留言三项，零值显示动作名、有值显示数量，未显示作者「更多操作」。

### 2026-09-09 客户视角详情本地编译入口

- `project.private.config.json` 新增「客户视角详情」编译模式，使用已验证可读取的作品 ID `2094718609789456385` 并传入 `owner=0`，用于直接核对客户视角底栏；仅影响本地微信开发者工具配置。
- `owner=0` 是显式客户视角：即使当前登录账号恰好是该作品作者，也必须展示客户底栏；未传 `owner` 参数的既有入口仍按原有身份判断。

### 2026-09-09 作品详情媒体背景统一为纯白

- 作者与客户视角共用的图片轮播、视频与 PDF 媒体容器背景统一为 `#FFFFFF`，移除媒体内容两侧的浅灰底；导航、描述区和底部操作栏原有白色样式保持不变。
- 验证：作品详情共享背景定向测试通过，`git diff --check` 通过；尚未完成本次开发者工具视觉验收。
...
### 2026-09-08 用户轨迹意向标签统一

- 修复轨迹页标签固定为红字粉底的问题：ViewModel 增加 intentLevel，映射时同步生成等级与文案，WXML 按等级绑定样式。
- 配色、40rpx 高度、22rpx 字号、12rpx 水平内边距与 48rpx 圆角对齐外层用户详情阅读记录标签。
- 验证：5 项用户轨迹测试通过，增加高/中/低等级与文案映射检查；git diff --check 通过。尚未完成本次真机视觉检查。


### 2026-09-08 客户视角底部分享栏去除分隔线

- 按用户截图移除 material-detail__share-bar 顶部边线，仅影响客户视角；作者操作栏及分享按钮自身描边保留。
- 更新既有样式断言；未进行本次真机视觉验收。


### 2026-09-08 开发者工具使用线上开发接口

- 开发者工具的 DEVTOOLS_ORIGIN 与线上主机一致时，请求 /dev/api，避免请求正式版 /api；局域网主机仍使用 /api。复用已有同源判断与开发接口基址函数。
- 正式版、体验版与真机开发环境选择不变，保留用户已有 config/dev.ts 配置修改。
- 验证：api-environment.test.mjs 的 3 项环境选择测试通过，覆盖开发工具线上/本地、正式版、体验版与真机开发。需重新编译小程序使内存中的基址缓存失效；未发起线上写请求。
...
| 会员套餐与微信支付 | done | 正式价：标准 ¥38.88 / ¥88.88 / ¥168.88，尊享 ¥80.88 / ¥208.88 / ¥388.88；六档 id：month / quarter / half_year / month_pro / quarter_pro / half_year_pro；经小程序虚拟支付开通。档位写入 `sales_user.member_tier` |
...
| PDF 点击预览图查看 | done | 按 Figma `1230:3417`，PDF 预览底部显示「点击查看」和提示文案；点预览或按钮进入阅读页 |
...
### 2026-09-04：首页顶部灰色渐变在滚动表面绘制（已于 2026-09-11 替换）
...
- 该灰色渐变与 `--home-header-gradient-opacity` 已于 2026-09-11 移除，首页改为使用本地彩色 PNG 顶部视觉；`home-header-background.svg` 仍不再使用。
...
- 尊享卡背景已由后续 Figma `1257:6281` 暖金色版本替换；文案仍为「尊享会员」「你是尊贵的尊享会员，享无限追踪人数」和「续费」。
...
- 三个 Pro 档位正式价：¥80.88 / ¥208.88 / ¥388.88。
- iOS 虚拟支付最低 1 元；需打开「苹果支付」、配置小程序简称。微信后台道具标价须与后端 `amountFen` 一致并发布到现网。
