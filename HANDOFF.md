...
- 通知、分析和素材发布入口的固定顶部 Header 使用白色实色端与白色透明端。素材页和首页内嵌素材 Tab 已删除彩色 `publish-header-gradient.png` 图层，保留 136px 高的白色渐变：顶部至 65.141% 为 `#FFFFFF`，底部过渡为透明，使素材列表只在标题区下方逐渐露出。
