...
- 通知和分析的固定顶部 Header 使用白色实色端与白色透明端。素材页和首页内嵌素材 Tab 已删除彩色 `publish-header-gradient.png` 图层，使用 136px 高的直线白色渐变：顶部 `#FFFFFF`（100%），底部 `rgba(255, 255, 255, 0)`（0%），使素材列表从顶部白色遮罩下逐渐露出。
