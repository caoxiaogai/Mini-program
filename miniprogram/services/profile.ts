...
export function getProfilePageData(): Promise<ProfilePageViewModel> {
  return ensureLogin().then((user) => {
    ...
    return Promise.all([prepareMediaUrl(avatarSource), getMembershipStatusSilent()]).then(([avatarUrl, membership]) => ({
      ...
      balanceLabel: '我的余额',
      withdrawLabel: '提现',
      membershipTitle: membership?.active ? '言界阿乐会员' : '解锁言界阿乐会员',
      membershipSubtitle:
        membership?.active && membership.expireLabel
          ? `有效期至 ${membership.expireLabel}`
          : '专属分析功能，不漏掉任何潜在用户',
      membershipActionLabel: membership?.active ? '续费会员' : '开通会员',
      pendingTitle: '尽情期待',
