...
test('home data layer exposes the new typed service seam', () => {
  ...
  assert.match(service, /path: '\/analysis\/dashboard'[\s\S]*silent: true/)
  assert.match(service, /path: '\/analysis\/customer\/list'[\s\S]*silent: true/)
  assert.match(service, /path: '\/analysis\/content\/list'[\s\S]*silent: true/)
  assert.match(service, /path: '\/analysis\/intent\/list'[\s\S]*silent: true/)
  assert.match(service, /path: '\/analysis\/notify\/list'[\s\S]*silent: true/)
...
test('entry pages require authorized login and first-time profile setup', async () => {
  const { isLoginProfileComplete, isLocalAvatarFile, safeReturnPath, buildAuthPath, buildShareGatePath, buildReturnPath } = await import('../miniprogram/utils/auth.ts')
  ...
  const shareGateLogic = read('miniprogram/pages/share-gate/index.ts')
  ...
  assert.ok(app.pages.includes('pages/share-gate/index'))
  ...
  assert.match(buildAuthPath('/pages/index/index?materialId=12'), /return=/)
  assert.match(buildShareGatePath('/pages/index/index'), /pages\/share-gate\/index/)
  ...
  assert.match(authService, /export function hasCompletedLogin/)
  assert.match(authService, /if \(hasCompletedLogin\(\)\) return Promise.resolve\('ok'\)/)
  assert.match(authService, /if \(!hasAuthorizedLogin\(\)\) return Promise.resolve\('login'\)/)
  assert.match(authService, /buildAuthPath\(returnPath\)/)
  assert.match(authService, /export function requireAccountLogin/)
  assert.match(authService, /logoutToAuth[\s\S]*HOME_PAGE_PATH/)
  ...
  assert.match(authPage, /onContinueNicknameTap\(/)
  assert.doesNotMatch(authMarkup, /agreePrivacyAuthorization/)
  assert.doesNotMatch(authMarkup, /查看协议/)
  assert.match(authPage, /onChooseAvatar\(/)
  ...
  assert.match(homeLogic, /startGuestHome\(options\)/)
  assert.match(homeLogic, /this\.startHome\(options\)/)
...
test('guest home requires login only for publish and membership', () => {
  const logic = read('miniprogram/pages/index/index.ts')
  const markup = read('miniprogram/pages/index/index.wxml')
  const authService = read('miniprogram/services/auth.ts')
  const shareGateLogic = read('miniprogram/pages/share-gate/index.ts')

  assert.match(logic, /startGuestHome\(options\)/)
  assert.match(logic, /if \(!hasCompletedLogin\(\)\) \{\s*this\.startGuestHome\(options\)/)
  assert.match(logic, /onMaterialPublishTap\(\) \{[\s\S]*requireLoginForAction\(\)/)
  assert.match(logic, /onProfileMembershipTap[\s\S]*requireLoginForAction\(/)
  assert.match(logic, /onHomeMembershipLimitTap[\s\S]*requireLoginForAction\(/)
  assert.match(logic, /onProfileSettingsTap\(\) \{[\s\S]*requireLoginForAction\('\/pages\/settings\/index'\)/)
  assert.match(logic, /onProfileIdentityTap\(\) \{[\s\S]*requireLoginForAction\(homeProfileTabUrl\(\)\)/)
  assert.match(markup, /identity-login="\{\{guestPreview\}\}" bind:identitytap="onProfileIdentityTap"/)
  assert.match(authService, /export function requireAccountLogin/)
  assert.match(shareGateLogic, /if \(this\.materialId\)/)
  assert.match(shareGateLogic, /HOME_MATERIALS_TAB_PATH/)
  assert.doesNotMatch(markup, /guest-guard/)
  assert.match(logic, /onTabTap\(event: WechatMiniprogram\.CustomEvent<\{ id\?: HomeTabId \}>\) \{\s*const id =/)
  assert.match(logic, /onPlusTap\(\) \{\s*this\.setActiveTab\(2\)\s*}/)
})
...
test('home bottom navigation switches primary pages without a swipe container', () => {
  ...
  assert.match(page, /<home-profile profile="\{\{profileData\}\}" identity-login="\{\{guestPreview\}\}" bind:identitytap="onProfileIdentityTap" bind:settingstap="onProfileSettingsTap" bind:membershiptap="onProfileMembershipTap" \/>/)
...
test('settings page shows intent rules in a dismissible dialog', () => {
  ...
  assert.match(settingTypes, /未看完所有图片为低意向，看完所有图片为中意向，查看2次及以上且至少1次看完所有图片、看完所有图片后又滑动看了前面的图片，或者转发1次及以上为高意向/)
  assert.match(settingTypes, /label: 'PDF'/)
  assert.match(settingTypes, /label: '笔记'/)
  assert.match(settingTypes, /未看完为低意向，看完为中意向，查看2次及以上且至少1次看完、看完后又向上翻浏览之前的内容，或者转发1次及以上为高意向/)
  assert.match(settingTypes, /label: '视频'/)
  assert.match(settingTypes, /播放进度低于80%为低意向，播放进度不低于80%为中意向，查看2次及以上且至少1次播放进度不低于80%、播放进度超过80%后又回退观看之前的内容，或者转发1次及以上为高意向/)
...
test('home renders the Figma 1055:2593 hidden-visitor prompt inside interaction messages when visitors are hidden', () => {
  ...
  assert.match(page, /class="home-membership-limit"><membership-limit-prompt variant="visitor" visitor-count="\{\{homeData\.limitPromptVisitorCount\}\}" visitor-description="\{\{homeData\.limitPromptDescription\}\}" visitor-avatars="\{\{homeData\.limitPromptVisitorAvatars\}\}" bind:upgrade="onHomeMembershipLimitTap" \/>/)
  ...
  assert.match(prompt, /\{\{visitorDescription\}\}/)
  assert.match(prompt, /wx:for="\{\{visitorAvatars\}\}"[\s\S]*?wx:key="id"/)
  assert.match(styles, /\.home-membership-limit\s*\{[\s\S]*?min-height: 63px;/)
  ...
  assert.match(service, /limitPromptDescription: visitorLimitPromptDescription\(membershipAccess\.tier, membershipAccess\.visitorLimit\)/)
  ...
  assert.match(promptService, /hiddenVisitorCount/)
  assert.doesNotMatch(promptService, /SHOW_VISITOR_LIMIT_PROMPT_PREVIEW/)
  assert.doesNotMatch(promptService, /figma-limit-visitor/)
  assert.match(service, /buildVisitorLimitPromptViewModel\(membershipAccess\)/)
...
test('notification surfaces reuse the Figma hidden-visitor membership prompt with 20px spacing', () => {
  ...
  assert.match(notificationPage, /wx:if="\{\{notifications\.showVisitorLimitPrompt\}\}"[\s\S]*class="notification-membership-limit"[\s\S]*?<membership-limit-prompt style="display: block; min-height: 63px;" variant="visitor" visitor-count="\{\{notifications\.limitPromptVisitorCount\}\}" visitor-description="\{\{notifications\.limitPromptDescription\}\}" visitor-avatars="\{\{notifications\.limitPromptVisitorAvatars\}\}" bind:upgrade="onMembershipLimitUpgrade" \/>/)
  assert.match(embeddedNotifications, /wx:if="\{\{notifications\.showVisitorLimitPrompt\}\}"[\s\S]*class="notification-membership-limit"[\s\S]*?<membership-limit-prompt style="display: block; min-height: 63px;" variant="visitor" visitor-count="\{\{notifications\.limitPromptVisitorCount\}\}" visitor-description="\{\{notifications\.limitPromptDescription\}\}" visitor-avatars="\{\{notifications\.limitPromptVisitorAvatars\}\}" bind:upgrade="onMembershipLimitUpgrade" \/>/)
  assert.match(prompt, /\{\{visitorDescription\}\}/)
  ...
  assert.match(read('miniprogram/services/notifications.ts'), /showVisitorLimitPrompt: limitPrompt\.visitorCount > 0/)
  assert.match(read('miniprogram/pages/notifications/notifications.ts'), /limitPromptTargetTier \?\? 'standard'/)
...
test('backend datetime strings display as China wall-clock hours', async () => {
  ...
  const { formatCompactCount, formatMonthDayTime, formatRelativeDayTime, parseDateTime } = await import('../miniprogram/utils/format.ts')
  ...
  assert.equal(formatCompactCount(1839), '1839')
  assert.equal(formatCompactCount(10000), '1万')
  assert.equal(formatCompactCount(98000), '9.8万')
  assert.equal(formatCompactCount(100000), '10万+')
  assert.equal(
...
test('notification incomplete status follows the work file type', async () => {
  ...
  assert.equal(buildNotificationStatus({ ...unread, fileType: 'NOTE' }), '未浏览完文件')
...
test('notification pages show a Figma mark-all action while the bottom navigation never shows an unread number', () => {
  ...
  assert.match(action, /catchtap="onCompactMarkAllReadTap"/)
  assert.match(action, /catchtap="onCompactCloseTap"/)
  const actionLogic = read('miniprogram/components/notification-mark-all-read/index.ts')
  assert.match(actionLogic, /onCompactCloseTap\(\) \{\s*if \(this\.data\.compact\) \{[\s\S]*return\s*\}\s*this\.triggerEvent\('markallread'\)/)
  assert.match(actionLogic, /onCompactMarkAllReadTap\(\) \{\s*if \(this\.data\.compact\) \{[\s\S]*\}\s*this\.triggerEvent\('markallread'\)/)
  assert.match(read('miniprogram/pages/notifications/notifications.ts'), /onMarkAllReadTap\(\) \{[\s\S]*persistViewedNotifications\(eventIds\)[\s\S]*markAllNotificationGroupsViewed/)
...
test('publish belongs to the root swiper and does not navigate to a separate materials page', () => {
  ...
  assert.match(publishLogic, /returnToEditedMaterial/)
  assert.match(read('miniprogram/utils/publish-return.ts'), /wx\.navigateBack/)
  assert.match(read('miniprogram/utils/publish-return.ts'), /returnToEditedMaterial/)
})
...
test('publish page picks image/video from camera or album and PDF from chat files', () => {
  ...
  assert.match(logic, /onAddMediaTap\(\) \{[\s\S]*this\.entryType === 'pdf'[\s\S]*choosePdfFromChat\(\)/)
  assert.match(logic, /onAddMediaTap\(\) \{[\s\S]*this\.entryType === 'video'[\s\S]*pendingMediaType = 'video'/)
  assert.match(logic, /onAddMediaTap\(\) \{[\s\S]*pendingMediaType = 'image'[\s\S]*publishSourceSheetVisible: true/)
  assert.doesNotMatch(logic, /onAddMediaTap\(\) \{[\s\S]*media\.length === 0[\s\S]*publishTypeSheetVisible: true/)
...
test('publish success modal shares to friends and guides moments from the detail page', () => {
  ...
  const materialsConfig = JSON.parse(read('miniprogram/pages/materials/index.json'))
  const publishLogic = read('miniprogram/pages/materials/publish/index.ts')
  const publishReturn = read('miniprogram/utils/publish-return.ts')
  const detailLogic = read('miniprogram/pages/material-detail/index.ts')
  const detailMarkup = read('miniprogram/pages/material-detail/index.wxml')
  const detailConfig = JSON.parse(read('miniprogram/pages/material-detail/index.json'))
  ...
  assert.match(modalMarkup, /分享给好友/)
  assert.match(modalMarkup, /分享到朋友圈/)
  assert.match(modalLogic, /onShareMomentsTap\(\) \{\s*showMomentsShareGuide\(\)/)
  assert.doesNotMatch(modalLogic, /sharefriends/)
  assert.match(publishLogic, /openCreatedMaterial\(\{[\s\S]*showSuccessModal: true/)
  assert.match(publishReturn, /buildMaterialDetailPath\(result\.materialId, undefined, true\)/)
  assert.match(publishReturn, /wx\.redirectTo/)
  ...
  assert.match(detailMarkup, /<publish-success-modal/)
  assert.match(detailLogic, /applyPendingPublishReturn\(\)/)
  assert.match(detailLogic, /revealPublishSuccessModal\(/)
  assert.match(detailLogic, /setData\(\{ detail,[\s\S]*this\.revealPublishSuccessModal\(detail\)/)
  const applyPending = detailLogic.match(/applyPendingPublishReturn\(\) \{[\s\S]*?\n  \},/)
  assert.ok(applyPending)
  assert.match(applyPending[0], /pendingPublishSuccess = true/)
  assert.doesNotMatch(applyPending[0], /showPublishSuccessModal: true/)
  assert.match(detailLogic, /revealPublishSuccessModal\(detail: MaterialDetailViewModel\) \{[\s\S]*showPublishSuccessModal: true/)
  assert.match(detailLogic, /showMomentsShareGuide|showPublishSuccessModal/)
  assert.match(read('miniprogram/pages/materials/index.ts'), /refreshAuthenticatedMaterials\(\) \{[\s\S]*this\.loadMaterials\(\)/)
  assert.match(homeLogic, /refreshAuthenticatedHome\(\) \{[\s\S]*takeOpenHomeProfileTab\(\)[\s\S]*this\.showProfileTab\(\)/)
  assert.match(homeLogic, /options\.tab === 'profile' \|\| takeOpenHomeProfileTab\(\)/)
  assert.match(homeLogic, /showProfileTab\(\) \{[\s\S]*setActiveTab\(index\)[\s\S]*loadProfileData\(\)/)
  assert.match(homeMarkup, /wx:if="\{\{activeTabIndex === 4\}\}"[\s\S]*home-profile/)
  assert.match(homeLogic, /refreshAuthenticatedHome\(\) \{[\s\S]*=== 'materials' \|\| takeMaterialsListNeedsRefresh\(\)/)
  ...
  assert.doesNotMatch(homeLogic, /onShareTimeline\(\)/)
  ...
  assert.equal(homeConfig.enableShareTimeline, undefined)
  assert.equal(materialsConfig.enableShareAppMessage, true)
  assert.equal(materialsConfig.enableShareTimeline, undefined)
...
test('share cards fill the WeChat 5:4 image area', () => {
  ...
  assert.doesNotMatch(
    materials.slice(
      materials.indexOf('export function prepareMaterialThumbnail'),
      materials.indexOf('export function prepareMaterialThumbnails'),
    ),
    /prepareShareCardImage/,
  )
  assert.match(detailLogic, /prepareShareCardImage\(previewUrl\)/)
  assert.match(detailLogic, /imageUrl: this\.resolveShareImageUrl\(detail\.previewUrl\)/)
  assert.match(homeLogic, /prepareShareCardImage\(previewUrl\)/)
  assert.match(materialsLogic, /prepareShareCardImage\(previewUrl\)/)
})
...
test('materials home uses the mine API and keeps the fixed Figma top layers', () => {
  ...
  assert.match(styles, /\.materials-page__header-gradient\s*\{[^}]*?position: absolute;[^}]*?top: 0;[^}]*?left: 0;[^}]*?z-index: 0;[^}]*?width: 100%;[^}]*?height: 136px;[^}]*?background: linear-gradient\(180deg, #ffffff 0%, #ffffff 80px, rgba\(255, 255, 255, 0\) 136px\);/)
...
test('material detail opens image preview, video player and PDF reader', () => {
  ...
  assert.match(markup, /detail\.fileType === 'PDF'[\s\S]*material-detail__pdf-open[\s\S]*点击查看/)
  assert.match(markup, /这是PDF文件，需要点击查看详情/)
  assert.match(read('miniprogram/pages/material-detail/index.less'), /\.material-detail__pdf-open-button[\s\S]*border-radius:\s*66rpx;[\s\S]*background:\s*#ffffff;/)
...
test('shared material opens the authorization gate before the detail page', async () => {
  ...
  assert.doesNotMatch(homeLogic, /buildMaterialShareTimelineQuery/)
...
test('material detail shares to friends from the forward action', () => {
  ...
  assert.match(markup, /class="material-detail-engage__action material-detail-engage__share"/)
  assert.doesNotMatch(markup, /分享给好友/)
  assert.doesNotMatch(markup, /分享到朋友圈/)
  assert.doesNotMatch(markup, /onShareMomentsTap/)
...
test('visitor material detail opens the existing comment sheet from the Figma comment entry', () => {
  ...
  assert.match(markup, /material-detail-comments__panel" bindtap="onCommentDismissKeyboard"/)
...
test('owner material detail puts delete and edit behind more while visitors only see engagement actions', async () => {
  ...
  assert.match(types, /likeCountLabel: string/)
  assert.match(types, /remoteUrl\?: string/)
  assert.match(service, /isOwner: forceVisitorView \? false : ownerView \|\| Boolean\(user && String\(material\.userId\) === String\(user\.userId\)\)/)
  ...
  assert.match(service, /path: `\/material\/\$\{materialId\}\/engagement`/)
  assert.match(service, /path: `\/material\/\$\{materialId\}\/like`/)
  assert.match(service, /path: `\/material\/\$\{materialId\}\/comments`/)
  assert.match(markup, /class="material-detail-engage \{\{detail\.isOwner/)
  ...
  assert.match(markup, /detail\.commentCountLabel/)
  assert.doesNotMatch(markup, /分享给好友/)
  assert.doesNotMatch(markup, /分享到朋友圈/)
  assert.doesNotMatch(markup, /material-detail-owner-bar/)
  assert.doesNotMatch(markup, /<text>好友<\/text>/)
  assert.equal(existsSync(new URL('../miniprogram/assets/materials/detail-like.svg', import.meta.url)), true)
  assert.equal(existsSync(new URL('../miniprogram/assets/materials/detail-forward.svg', import.meta.url)), true)
  assert.equal(existsSync(new URL('../miniprogram/assets/materials/detail-comment.svg', import.meta.url)), true)
  assert.match(markup, /bindtap="onOwnerMoreTap"/)
  assert.match(logic, /itemList: \['修改', '删除'\]/)
  assert.match(logic, /buildMaterialEditPath\(detail\.id, detail\.fileType === 'NOTE' \? 'note' : ''\)/)
  ...
  assert.match(styles, /\.material-detail-engage--visitor \.material-detail-engage__actions \{[\s\S]*gap: 40rpx;/)
  ...
  assert.match(service, /export function getMaterialDetail\(/)
  assert.match(service, /isOwner: forceVisitorView \? false : ownerView \|\| Boolean\(user && String\(material\.userId\) === String\(user\.userId\)\)/)
  ...
  assert.match(publishLogic, /this\.draftMaterialId = draft\.id/)
  assert.match(publishLogic, /this\.draftMediaPaths = draft\.media\.map/)
  assert.match(publishLogic, /returnToEditedMaterial\(materialId\)/)
  assert.match(logic, /takeMaterialDetailNeedsRefresh\(\)/)
  assert.match(publishReturn, /export function returnToEditedMaterial/)
  assert.match(publishReturn, /markMaterialsListNeedsRefresh\(\)/)
  assert.match(publishReturn, /refreshStackedMaterialsLists\(\)/)
  assert.match(publishReturn, /pages\/material-detail\/index/)
  assert.match(read('miniprogram/pages/index/index.ts'), /buildMaterialDetailPath\(materialId, undefined, true\)/)
  assert.match(read('miniprogram/pages/materials/index.ts'), /buildMaterialDetailPath\(materialId, undefined, true\)/)
})
...
test('analysis work list re-sorts when the sort option changes', () => {
  ...
  assert.doesNotMatch(homeLogic, /capAudienceUsers/)
  assert.doesNotMatch(analysisLogic, /capAudienceUsers/)
...
test('analysis total tab follows Figma 587:8623 overview and peak layout', () => {
  ...
  assert.match(pageStyles, /\.analysis-total__overview-grid\s*\{[\s\S]*?grid-template-columns: repeat\(3, minmax\(0, 1fr\)\);[\s\S]*?gap: 10px;/)
  assert.match(pageStyles, /\.analysis-total__overview-item\s*\{[\s\S]*?width: 100%;[\s\S]*?min-width: 0;[\s\S]*?height: 63px;[\s\S]*?padding: 10px;[\s\S]*?border: 1px solid #ebebeb;[\s\S]*?border-radius: 12px;/)
  assert.doesNotMatch(pageStyles, /\.analysis-total__overview-grid\s*\{[\s\S]*?98px/)
  assert.doesNotMatch(pageStyles, /\.analysis-total__overview-item\s*\{[\s\S]*?width: 98px;/)
...
test('notification, analysis, and publish headers use the white top gradient', () => {
  ...
  assert.match(materialsStyles, /background: linear-gradient\(180deg, #ffffff 0%, #ffffff 80px, rgba\(255, 255, 255, 0\) 136px\);/)
...
test('share gate follows the latest compact social-proof and outlined-action design', () => {
  ...
  assert.match(markup, /以上好友赞了该内容/)
  assert.doesNotMatch(markup, /你有\s*<text class="share-gate-page__likes-count">6<\/text>\s*个好友赞了该内容/)
  assert.match(styles, /\.share-gate-page__hint\s*\{[\s\S]*color:\s*#666666;[\s\S]*font-size:\s*14px;/)
  assert.match(styles, /\.share-gate-page__likes\s*\{[\s\S]*font-size:\s*16px;[\s\S]*white-space:\s*nowrap;/)
  assert.match(styles, /\.share-gate-page__content\s*\{[\s\S]*gap:\s*20px;/)
  assert.match(styles, /\.share-gate-page__social-proof\s*\{[\s\S]*gap:\s*10px;[\s\S]*margin-top:\s*0;/)
  assert.match(styles, /\.share-gate-page__avatar\s*\{[\s\S]*filter:\s*blur\(3px\);/)
  ...
  assert.match(styles, /\.share-gate-page__primary\s*\{[\s\S]*display:\s*block;[\s\S]*width:\s*100%;[\s\S]*height:\s*48px;[\s\S]*margin:\s*0;[\s\S]*border:\s*1px solid #e0e0e0;[\s\S]*color:\s*#ff8901;[\s\S]*background:\s*#f0f0f0;[\s\S]*line-height:\s*48px;/)
})

test('share gate hero removes legal copy and keeps the primary content 32px above the screen bottom', () => {
  const markup = read('miniprogram/components/share-gate-view/index.wxml')
  const styles = read('miniprogram/components/share-gate-view/index.less')
  ...
  assert.match(styles, /\.share-gate-page\s*\{[\s\S]*background:\s*#f8f8f8;/)
  assert.match(styles, /\.share-gate-page\s*\{[\s\S]*position:\s*relative;/)
  assert.match(styles, /\.share-gate-page__hero\s*\{[\s\S]*position:\s*absolute;[\s\S]*top:\s*0;[\s\S]*right:\s*0;[\s\S]*bottom:\s*0;[\s\S]*left:\s*0;[\s\S]*padding:\s*0;/)
  assert.match(styles, /\.share-gate-page__content\s*\{[\s\S]*flex:\s*0 0 400px;[\s\S]*height:\s*400px;[\s\S]*padding:\s*32px 20px /)
  assert.match(styles, /\.share-gate-page__content\s*\{[\s\S]*position:\s*relative;[\s\S]*z-index:\s*1;[\s\S]*margin-top:\s*auto;/)
  assert.match(styles, /\.share-gate-page__content\s*\{[\s\S]*padding:\s*32px 20px 32px;/)
  assert.doesNotMatch(styles, /padding:\s*32px 20px calc\(32px \+ env\(safe-area-inset-bottom\)\)/)
  assert.match(styles, /\.share-gate-page__content\s*\{[\s\S]*background:\s*linear-gradient\(to top, #ffffff 0, #ffffff 180px, rgba\(255, 255, 255, 0\) 400px\);/)
  ...
  assert.match(styles, /\.share-gate-page__art\s*\{[\s\S]*position:\s*absolute;[\s\S]*top:\s*0;[\s\S]*right:\s*0;[\s\S]*bottom:\s*0;[\s\S]*left:\s*0;[\s\S]*width:\s*100vw;[\s\S]*height:\s*100vh;/)
  assert.doesNotMatch(markup, /share-gate-page__headline|好产品|值得多了解一点/)
  assert.doesNotMatch(styles, /share-gate-page__headline/)
  assert.doesNotMatch(markup, /share-gate-page__legal|微信安全协议保护/)
  assert.doesNotMatch(styles, /share-gate-page__legal/)
  assert.match(markup, /<navigation-bar[^>]*background="transparent"/)
  assert.doesNotMatch(markup, /share-gate-page__graphic/)
  assert.match(markup, /share-gate-page__art/)
  ...
  const componentLogic = read('miniprogram/components/share-gate-view/index.ts')
  const defaultBackgroundPath = 'miniprogram/assets/share-gate/default-background.png'
