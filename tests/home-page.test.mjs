...
test('home page renders a replaceable theme image under a 380px white fade', () => {
  ...
  assert.match(page, /<image class="home-page__theme-image" src="\/assets\/home-new\/home-theme-sunset\.png" mode="aspectFill" \/>/)
...
test('materials home uses the mine API and keeps the fixed Figma top layers', () => {
  ...
  assert.doesNotMatch(service, /from '\.\.\/mocks\//)
  assert.match(markup, /<view class="materials-page__top">[\s\S]*?<image class="materials-page__header-art" src="\/assets\/materials\/publish-header-gradient\.png" mode="scaleToFill" \/>[\s\S]*?<view class="materials-page__header">/)
  assert.match(homeMarkup, /<view class="materials-page__top">[\s\S]*?<image class="materials-page__header-art" src="\/assets\/materials\/publish-header-gradient\.png" mode="scaleToFill" \/>[\s\S]*?<view class="materials-page__header">/)
  ...
  assert.match(styles, /\.materials-page__header-art\s*\{[^}]*?position: absolute;[^}]*?top: 0;[^}]*?left: 0;[^}]*?z-index: 0;[^}]*?width: 100%;[^}]*?height: 136px;/)
...
test('notification and analysis headers use a white top gradient while publish uses its header art', () => {
  ...
  assert.match(materialsMarkup, /<image class="materials-page__header-art" src="\/assets\/materials\/publish-header-gradient\.png" mode="scaleToFill" \/>/)
...
test('home hero removes its grey top gradient and uses the theme layers', () => {
  ...
  assert.match(markup, /<scroll-view[^>]*class="home-page__tab-scroll"[^>]*>[\s\S]*?<image class="home-page__theme-image" src="\/assets\/home-new\/home-theme-sunset\.png" mode="aspectFill" \/>[\s\S]*?<view class="home-page__theme-fade" \/>[\s\S]*?<navigation-bar back="\{\{false\}\}" title="首页"/)
