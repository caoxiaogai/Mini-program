...
test('materials home uses the mine API and keeps the fixed Figma top layers', () => {
  ...
  assert.match(styles, /\.materials-page__header-gradient\s*\{[^}]*?position: absolute;[^}]*?top: 0;[^}]*?left: 0;[^}]*?z-index: 0;[^}]*?width: 100%;[^}]*?height: 136px;[^}]*?background: linear-gradient\(180deg, #ffffff 0%, #ffffff 65\.141%, rgba\(255, 255, 255, 0\) 100%\);/)
...
test('notification, analysis, and publish headers use the white top gradient', () => {
  ...
  assert.match(materialsStyles, /background: linear-gradient\(180deg, #ffffff 0%, #ffffff 65\.141%, rgba\(255, 255, 255, 0\) 100%\);/)
