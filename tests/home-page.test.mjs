...
test('publish success modal shares to friends only', () => {
  ...
  assert.doesNotMatch(modalMarkup, /分享到朋友圈/)
  assert.doesNotMatch(modalMarkup, /onShareMomentsTap/)
  assert.doesNotMatch(modalLogic, /sharefriends/)
  assert.doesNotMatch(modalLogic, /sharemoments/)
  ...
  assert.doesNotMatch(homeMarkup, /sharemoments/)
  ...
  assert.doesNotMatch(homeLogic, /showMomentsShareGuide/)
...
test('owner material detail puts delete and edit behind more while visitors only see engagement actions', async () => {
  ...
  assert.match(logic, /itemList: \['二次编辑', '删除'\]/)
