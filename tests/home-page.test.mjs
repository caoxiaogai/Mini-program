...
test('devtools profile can preview the selected membership card without changing real membership status', () => {
  const devConfig = read('miniprogram/config/dev.ts')
  const profileService = read('miniprogram/services/profile.ts')

  assert.match(devConfig, /DEVTOOLS_PROFILE_MEMBERSHIP_CARD: 'standard' \| 'premium' \| '' = 'premium'/)
  assert.match(profileService, /DEVTOOLS_PROFILE_MEMBERSHIP_CARD/)
  assert.match(profileService, /wx\.getSystemInfoSync\(\)\.platform !== 'devtools'/)
  assert.match(profileService, /previewCard === 'premium'/)
  assert.match(profileService, /expireLabel: '2026\.11\.20'/)
})
