...
test('owner material detail puts delete and edit behind more while visitors only see engagement actions', async () => {
  ...
  assert.match(markup, /wx:if="\{\{detail\.isOwner\}\}"[\s\S]*note-more\.svg/)
  ...
  assert.match(styles, /\.material-detail-engage \{[\s\S]*border-top: 1px solid #f0f0f0;[\s\S]*background: @app-page-background;/)
  assert.match(styles, /\.material-detail-engage__action \{[\s\S]*flex: 1 1 0;/)
  assert.match(styles, /\.material-detail-engage__more \{[\s\S]*flex: none;/)
