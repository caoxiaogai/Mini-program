...
test('materials home uses the mine API and keeps the fixed Figma top layers', () => {
  ...
  assert.match(styles, /\.materials-page__gradient\s*\{[\s\S]*?position: fixed;[\s\S]*?z-index: 2;[\s\S]*?height: 131px;[\s\S]*?background: linear-gradient\(180deg, #f5f5f5 0%, #f5f5f5 65\.141%, rgba\(245, 245, 245, 0\) 100%\);/)
