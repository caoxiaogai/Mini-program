...
test('note editor can add text, location, image, video, file and save like other materials', () => {
  ...
  assert.match(markup, /placeholder="\{\{emptyHint\}\}"/)
  assert.match(logic, /点此编辑文字/)
  assert.match(logic, /NOTE_EMPTY_HINT/)
  assert.match(styles, /\.note-block__placeholder\s*\{[\s\S]*color:/)
  assert.match(markup, /scroll-into-view/)
  assert.match(markup, /scroll-top="\{\{scrollTop\}\}"/)
  assert.match(markup, /note-caret-\{\{item.id\}\}/)
  assert.match(markup, /bindlinechange="onTextLineChange"/)
  assert.match(logic, /ensureCaretVisible/)
  assert.match(logic, /scheduleEnsureCaretVisible/)
  assert.match(logic, /NOTE_CARET_SCROLL_MS/)
  assert.match(logic, /estimateCaretLines/)
  ...
  assert.match(markup, /note-block--text-lead/)
  assert.match(markup, /note-page__focus-pad[^\n]*bindtap="onEditorBlankTap"/)
  assert.match(styles, /\.note-block--text-lead\s*\{[\s\S]*min-height:\s*144rpx;/)
  assert.match(styles, /\.note-page__focus-pad\s*\{[\s\S]*height:\s*144rpx;/)
  assert.match(markup, /onPlusTap/)
  assert.match(markup, />完成</)
  assert.match(markup, /catchtap="onDoneTap"/)
  assert.match(logic, /onDoneTap\(\) \{[\s\S]*this\.dismissKeyboard\(\)/)
  ...
  assert.match(markup, /\{\{submitLabel\}\}/)
  assert.doesNotMatch(markup, /存草稿/)
  assert.match(markup, /<upload-loading visible="\{\{uploading\}\}"/)
  assert.match(logic, /submitLabel: '修改'/)
  ...
  assert.match(logic, /onEditorBlankTap\(\) \{[\s\S]*editorActive \|\| this\.data\.textFocused/)
  assert.match(logic, /alreadyFocused/)
  assert.match(logic, /NOTE_KEYBOARD_DISMISS_MS/)
  assert.match(logic, /NOTE_KEYBOARD_DISMISS_MS = 720/)
  assert.match(logic, /applyKeyboardHeight\(height: number\) \{\s*if \(this\.openingPlusPanel\) \{/)
  ...
  assert.match(logic, /NOTE_LOCATION_FALLBACK/)
  ...
  assert.match(logic, /openCreatedMaterial\(\{[\s\S]*showSuccessModal: true/)
  ...
  assert.match(detailLogic, /buildMaterialEditPath\(detail.id, detail.fileType === 'NOTE' \? 'note' : ''\)/)
