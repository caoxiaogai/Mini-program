...
test('publish type selection asks camera or album before opening the native picker', async () => {
  ...
  assert.match(picker, /wx\.requirePrivacyAuthorize\(/)
  assert.match(picker, /showPublishPickerError\(errMsg\?: string\): void \{[\s\S]*wx\.showModal\(\{[\s\S]*content: \(errMsg \?\? ''\)\.trim\(\) \|\| '无错误信息'/)
  assert.doesNotMatch(picker, /选择失败，请稍后重试/)
  assert.match(homeLogic, /onPublishSourceSelect[\s\S]*this\.setData\(\{ publishSourceSheetVisible: false \}\)\s*this\.openPublishEditorFromPicker\(type, source\)/)
  assert.match(homeLogic, /openPublishEditorFromPicker\(type, source\)/)
  assert.match(homeLogic, /skipNextShowRefresh = true/)
  assert.match(homeLogic, /if \(this\.skipNextShowRefresh\)/)
  ...
  assert.match(materialsLogic, /skipNextShowRefresh = true/)
  assert.match(materialsLogic, /if \(this\.skipNextShowRefresh\)/)
...
test('publish image grid uses a finger-following floating preview and swaps only on release', () => {
  ...
  assert.match(logic, /dragTargetSlotIndex,\r?\n    \}\)/)
...
test('publish actions match Figma 850:9374 without a white bottom container', () => {
  ...
  assert.doesNotMatch(styles, /publish-page__draft-button/)
})
...
test('draft and publish buttons show a file-upload overlay until MinIO uploads finish', () => {
  ...
  assert.doesNotMatch(publishLogic, /saveMaterialDraft/)
  assert.match(publishLogic, /this\.uploadThenSubmit\(\(input\) =>\s*publishMaterial\(input\)/)
  assert.doesNotMatch(noteLogic, /saveNoteDraft/)
  ...
  assert.match(publishMarkup, /\{\{submitLabel\}\}/)
  assert.match(publishLogic, /submitLabel: '修改'/)
