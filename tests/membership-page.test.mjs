...
test('membership page is registered and uses the typed service seam', () => {
  ...
  assert.match(logic, /pairedPlanId/)
...
test('membership page defaults to the Figma-selected three-month plan', async () => {
  ...
  const { pairedPlanId } = await import('../miniprogram/utils/membership.ts')
  assert.equal(pairedPlanId('month', 'premium'), 'month_pro')
  assert.equal(pairedPlanId('quarter_pro', 'standard'), 'quarter')
  assert.equal(pairedPlanId('half_year_pro', 'premium'), 'half_year_pro')
})
